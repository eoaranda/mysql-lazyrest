...
| GET | `<table_name>`/limit/`<num>` | 200 | Returns the first <n> number of rows of the request table |
| GET | `<table_name>`/order/`<col_name>`/`<asc_desc>`/limit/`<num>` | 200 | Returns  the first <n> number of rows of a requested table ordered by a requested column and in asc or desc order |
